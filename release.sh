...
if [[ -z "$VERSION" ]]; then
  ...
  echo "Example: ./release.sh 0.2.4"
  exit 1
fi

# Reject leading 'v' — VERSION is the semver, TAG gets the prefix
if [[ "$VERSION" == v* ]]; then
  echo "Error: pass the version without the leading 'v' (got '${VERSION}'). Example: ./release.sh 0.2.4"
  exit 1
fi
...
if [[ "${CURRENT_VERSION}" == "${VERSION}" ]]; then
  ...
  git add pom.xml btcpay-java-core/pom.xml btcpay-java-spring-boot-starter/pom.xml btcpay-java-examples/pom.xml README.md
...
echo "https://github.com/AskAHumanOnline/btcpay-java/actions"
